...
| `No status received from the device within 300s`                                                   | Subscription accepted, nothing ever sent: device off/renumbered, wrong `host`/`port`, or **another connection already holds the device**. Since 1.2.0-beta.5 this cannot be a purifier that reported itself switched off — that case is silent                                         | `ps -eo pid,ppid,args \| grep [a]ioairctrl` for a competing process (a leftover from ≤ v1.1.0 has PPID 1); then `ping` the device                                         | Kill the competitor / fix the config. Purifiers serve one connection at a time                                                                                                                    |
...
| `The device did not apply <key>=<value> after 2 attempts`                                          | The write was transmitted and never applied: a lost `NON` packet, or the device asleep. The plugin already resent it once                                                                                                                                                              | With `debug: true`, check the `resending <key>=` line before it, then run the logged `CMD:` line manually as `<hbuser>`                                                   | Usually transient. Persistent means the register/value is wrong for this model → `new-model-support-campaign`                                                                                     |
