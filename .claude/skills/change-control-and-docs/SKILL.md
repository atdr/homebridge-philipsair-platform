...
| **Anything a skill in `.claude/skills/` states as fact** — commands, flags, log strings, timings, file paths, invariants, model data | The affected `SKILL.md`, same PR. Run each skill's "Provenance and maintenance" commands to find drift | Not enforced — this row is the enforcement                    |
