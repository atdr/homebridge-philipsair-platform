...
`aioairctrl got no answer from <host>:<port> within 12s and was stopped.` Sending a command is not a single packet: the CLI opens a session with the device first, and if nothing answers, that wait never ends by itself. The plugin therefore stops the attempt, reports it, and puts the Home app back to the last state the device actually reported, rather than leaving a switch showing a command that went nowhere. The cap is set a few seconds above HomeKit's own ~9s write timeout on purpose, so HomeKit will already have shown the accessory as Not Responding for a few seconds by the time this message fires, and the correction that follows lands in a window where the Home app will actually accept it.
