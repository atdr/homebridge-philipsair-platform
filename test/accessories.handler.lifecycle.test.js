...
describe('write verification', { concurrency: 1 }, () => {
  ...
  it("caps a set command below HAP's own write timeout by default", () => {
    const handler = makeHandler({});
    ...
    //only pins that the *default* itself stays under that ceiling (issue #83)
    assert.equal(handler.sendTimeout, 8000);
