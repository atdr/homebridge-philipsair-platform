...
const workflow = fs.readFileSync(path.join(__dirname, '..', '.github', 'workflows', 'publish-prerelease.yml'), 'utf8');
...
describe('prerelease workflow', () => {
  it('requires id-token: write to authenticate to npm', () => {
    ...
    assert.match(
      ...
      /id-token: write/,
      'publish-prerelease cannot authenticate to npm without id-token: write'
    );
  });

  it('only runs on workflow_dispatch', () => {
  ...
  it('grants write access only to the job that creates the tag', () => {
    ...
    assert.match(
      jobs.get('publish-prerelease'),
      /contents: read/,
      'publish-prerelease should not hold a writable token while publishing'
    );
  });
});
